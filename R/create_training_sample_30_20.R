...
phl20$age <- recode(phl20$age, 
                    ...
                    `115 (115+ in the 1980 internal data)` = "115",
...
names(phl20) <- paste0(names(phl20), "1")
names(phl30) <- paste0(names(phl30), "2")


# Create 10 sets of training data drawing from 100 random household heads in 1930
...
for(x in 0:9){
  ...
  for(a in 1:nrow(chunk)){
    ...
    block <- filter(phl20, byear1 <= byearmax & byear1 >= byearmin & sex1 == sex & bpl1 == bpl)
    ...
    block <- filter(block, jw_frst >= 0.75 & jw_last >= 0.8) %>%
      ...
      # 0.6 adds a lot of junk (~1200 potential matches for 86 of 100 records from X1)
      ...
      # 0.8 gives 300 potential matches for 73 of 100 records from X1
  ...
  # output the file
  write_csv(training_set, here("data", "Uncoded", paste0("training_30_20_", x, ".csv")))
